...
class ai16zTrailingStop(ScriptStrategyBase):
    ...
    def __init__(self, connectors: Dict[str, Dict[str, str]]):
        ...
        self.quote_token_address = "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v"  # USDC mint address
    ...
    def init_event_listeners(self):
        ...
        try:
            ...
            self.market.add_listener(MarketEvent.SellOrderCompleted, self.sell_fill_forwarder)
            self.market.add_listener(MarketEvent.BuyOrderCompleted, self.buy_fill_forwarder)
            self.logger.info("Event listeners initialized successfully")
        except Exception as e:
            self.logger.error(f"Error initializing event listeners: {str(e)}", exc_info=True)
    ...
    async def check_initial_balance(self):
        ...
        if balances and "balances" in balances:
            ...
            self.logger.info(f"Initial balances: {base_balance} {base_token}, {quote_balance} {quote_token}")
            ...
            if base_balance > 0:
                ...
                self.logger.error(f"Insufficient {base_token} balance to start strategy.")
                return False
        else:
        ...
        try:
            ...
            try:
                ...
                if sell_price_response and "price" in sell_price_response:
                    ...
                    self.logger.info(f"Current price: {current_price:.6f} USDC per AI16Z")
                    
                    if not self.is_tracking:
                        self.start_tracking(current_price)
                    else:
                        self.logger.info(f"Highest price: {self.highest_observed_price} USDC")
    ...
    def update_trailing_stop(self, current_price: Decimal):
        ...
        if current_price <= self.stop_price:
            ...
            self.logger.warning(f"Current price ({current_price}) <= Stop price ({self.stop_price})")
        ...
        if current_price > self.highest_observed_price:
            ...
            self.logger.info(f"New stop price: {self.stop_price} USDC (was: {old_stop} USDC)")
        ...
        try:
            ...
            if balances and "balances" in balances:
                ...
                if base_balance > Decimal("0"):
                    ...
                    self.logger.info(f"Expected output: {expected_output} {quote_token}")
                    
                    # Calculate minimum acceptable price with slippage
                    min_acceptable_price = current_price * (Decimal("1") - self.max_slippage_pct)
                    self.logger.info(f"Minimum acceptable price: {min_acceptable_price} {quote_token} per {base_token}")
                    
                    # Calculate minimum expected output with slippage
                    min_expected_output = amount * min_acceptable_price
                    self.logger.info(f"Minimum expected output (with {self.max_slippage_pct * 100}% slippage): {min_expected_output} {quote_token}")
                    ...
                    try:
                        ...
                        new_balances = await self.market._get_gateway_instance().get_balances(
                            self.market.chain,
                            self.market.network,
                            self.market.address,
                            [base_token, quote_token]
                        ...
                        if new_balances and "balances" in new_balances:
                            new_base_balance = Decimal(str(new_balances["balances"].get(base_token, 0)))
                            new_quote_balance = Decimal(str(new_balances["balances"].get(quote_token, 0)))
                            self.logger.info(f"Balance after order:")
                            self.logger.info(f"New {base_token} balance: {new_base_balance}")
                            self.logger.info(f"New {quote_token} balance: {new_quote_balance}")
                            ...
                            if base_sold > 0:
                                ...
                                self.logger.info(f"Order filled:")
                                self.logger.info(f"Amount sold: {base_sold} {base_token}")
                                self.logger.info(f"Amount received: {quote_received} {quote_token}")
                                self.logger.info(f"Actual price: {actual_price} {quote_token} per {base_token}")
                                self.logger.info(f"Price slippage: {((current_price - actual_price) / current_price * 100):.4f}%")
                            else:
                                self.logger.warning("Order might not have been filled (balance unchanged)")
                        
                    except Exception as order_error:
                        self.logger.error(f"Error placing order: {str(order_error)}", exc_info=True)
                else:
                    self.logger.warning(f"No {base_token} balance available for stop loss.")
            else:
    ...
    def stop(self, clock: Optional[Clock] = None):
        ...
        try:
            self.market.remove_listener(MarketEvent.OrderFilled, self.sell_fill_forwarder)
            self.market.remove_listener(MarketEvent.OrderFilled, self.buy_fill_forwarder)
            self.market.remove_listener(MarketEvent.SellOrderCompleted, self.sell_fill_forwarder)
            self.market.remove_listener(MarketEvent.BuyOrderCompleted, self.buy_fill_forwarder)
