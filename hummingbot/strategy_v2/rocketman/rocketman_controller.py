...
class RocketmanController(DirectionalTradingControllerBase):
    ...
    async def update_processed_data(self):
        try:
            ...
            # Get candles data from Birdeye
            candles_config = self.config.candles_config[
                0
            ]  # Use the first candles config
            self._logger.info(
                f"Getting candles from {candles_config.connector} for {candles_config.trading_pair}"
            )
            ...
            if candles_df is not None and not candles_df.empty:
                self._logger.info(f"Got candles data: {len(candles_df)} rows")
                ...
                self._logger.warning("No candles data available")
